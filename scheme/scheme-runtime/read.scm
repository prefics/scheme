...
(define (write* obj port depth)
  (cond ((number? obj)  (write-number obj port))
	...
	(else (port-write-string port "#{unprintable object}"))))
...
(define (write-vector vector port depth)
  ...
  (let loop ((i 0))
	...
	(if (and *write-depth* (< (+ depth i) *write-depth*))
	    (begin
	      (write* (vector-ref vector i) port (+ depth i))
...
(define (write-byte-vector vector port depth)
  ...
  (let loop ((i 0))
	...
	(if (and *write-depth* (< (+ depth i) *write-depth*))
	    (begin
	      (write* (bvec-ref vector i) port (+ depth i))
