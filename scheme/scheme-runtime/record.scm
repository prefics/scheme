...
(define (make-initial-type name supers direct-slots slots methods)
  (let ((type (make-stob 7 #f)))
    ...
    (if (not (= (length supers) 1))
        (error "An initial type only support single inheritence"))
    (stob-set! type 5 (cons type (stob-ref (car supers) 5)))
...
(define <integer> (make-initial-type '<integer> (list <number>) '() '() '()))
(define <real> (make-initial-type '<real> (list <number>) '() '() '()))
(define <collection> (make-initial-type '<collection> (list <top>) '() '() '()))
(define <sequence> (make-initial-type '<sequence> (list <collection>) '() '() '()))
(define <string> (make-initial-type '<string> (list <sequence>) '() '() '()))
(define <vector> (make-initial-type '<vector> (list <sequence>) '() '() '()))
(define <byte-vector> (make-initial-type '<byte-vector> (list <sequence>) '() '() '()))
(define <list> (make-initial-type '<list> (list <sequence>) '() '() '()))
(define <pair> (make-initial-type '<pair> (list <list>) '() '() '()))
(define <nil> (make-initial-type '<nil> (list <list>) '() '() '()))
